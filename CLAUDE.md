...
3. The thread runs the stages: extract audio + transcribe each take with word-level timestamps (`ai_service::transcribe_timed`) → build the planning prompt and call the LLM (`ai_service::complete`) → parse/validate the returned EDL against the real takes (clamp ranges to durations) → write the EDL JSON to `edits/production-<id>/` → extract a normalized segment per clip (burning in per-clip captions re-timed from the transcript when enabled) → concat with FFmpeg → optionally mix in looped background music (`ffmpeg_service::add_background_music`) → write the final `.mp4` to the chosen output folder/filename (or the app-data folder by default)
