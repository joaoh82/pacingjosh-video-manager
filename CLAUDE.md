...
3. The thread runs the stages: extract audio + transcribe each take with word-level timestamps (`ai_service::transcribe_timed`) → build the planning prompt and call the LLM (`ai_service::complete`) → parse/validate the returned EDL against the real takes (clamp ranges to durations) → extract a normalized segment per clip (burning in per-clip captions re-timed from the transcript when enabled) → concat with FFmpeg → optionally mix in looped background music, ducked under the voice via sidechain compression (`ffmpeg_service::add_background_music`). All output goes to `<output_dir>/<production title>/`: the final `.mp4`, its `.json` EDL (same basename), and a transient `.work-<job>/` that is removed on success. **Nothing is written to the app-data directory** — `output_dir` is required.
