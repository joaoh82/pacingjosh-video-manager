...
3. The thread runs the stages: extract audio + transcribe each take with word-level timestamps (`ai_service::transcribe_timed`) → build the planning prompt and call the LLM (`ai_service::complete`) → parse/validate the returned EDL against the real takes (clamp ranges to durations) → extract a normalized segment per clip (burning in per-clip captions re-timed from the transcript when enabled) → concat with FFmpeg → optionally mix in looped background music with two explicit levels — a `music_volume` for pauses and a lower `music_duck_volume` while talking — implemented as a constant floor + a voice-keyed sidechain duck of the "swing" layer (`ffmpeg_service::add_background_music`). All output goes to a per-run version folder `<output_dir>/<production title>/v<N>/` (N = max existing `v*` + 1, so re-edits never overwrite): the final `.mp4`, its `.json` EDL (same basename), and a transient `.work-<job>/` removed on success. **Nothing is written to the app-data directory** — `output_dir` is required. Deleting a run (`DELETE /api/edits/{id}`) removes the DB row and the files, plus the now-empty version folder.
