...
- **Components** (`components/`): VideoGrid, VideoCard, VideoModal (player + metadata editor + AI content panel), FilterPanel, Scanner (progress polling), ProductionManager, VideoEditPipeline (the Edit & Create Video modal), EditTimeline (interactive CapCut-style video/voice/music timeline built from `edl.timeline` — embeds a preview player fed by `GET /api/edits/{id}/video` whose playback drives a red playhead, with click/drag-the-ruler scrubbing; zoom/scroll. Post-render editing: select clip(s) → inspector to **trim** (drag clip edges or numeric in/out, clamped by `takeDurations`) / **remove** (restore from "Removed" chips) / toggle **voice enhancement**; the layout re-flows live off the pending edits and remaps the speech/music tracks; click a music burst for a Remove/Fade menu; **✨ Ask AI** posts to `POST /api/edits/{id}/ai-edit` and applies the returned plan. Re-render via `TimelineEdits` {clips: ClipEdit[], mute, fade}), ThumbnailEditor (canvas thumbnail builder — frame grab + text overlay + optional Gemini AI restyle), BulkActions.
