...
- **Routes** (`routes/`): Actix-web handlers — `videos.rs`, `scan.rs`, `tags.rs`, `productions.rs`, `stream.rs`, `config_routes.rs`, `ai.rs` (AI settings + per-video copy generation), `edit.rs` (the Edit & Create Video pipeline, incl. `GET /api/overlays/builtin` listing the bundled overlay snippets and `GET /api/overlays/preview?path=` serving an overlay image/GIF for the live placement preview), `search.rs` (semantic search — `GET /api/search/semantic?q=&type=videos|productions`, the background `POST /api/search/reindex` + `GET /api/search/reindex/status/{id}`, and `GET /api/search/index-status`). All mounted under `/api`.
- **Services** (`services/`): Business logic — `scanner.rs` (background directory scanning), `ffmpeg_service.rs` (metadata/thumbnails/clip extraction + concat + overlay compositing, with injectable `FfmpegPaths` via `set_ffmpeg_paths()`), `video_service.rs`, `search_service.rs` (keyword `LIKE` filtering + sort/paginate), `production_service.rs`, `ai_service.rs` (transcription incl. ElevenLabs/OpenAI/Gemini + LLM completion + `embed_texts` batch embeddings via OpenAI/Gemini + `describe_video_frames` vision captioning via the multimodal text provider; the OpenAI/Gemini completion paths request JSON mode; Anthropic has no `response_format` and **must not** prefill the assistant turn with `{` to fake one — newer models reject prefill with a 400 `invalid_request_error` and there is no capability flag to test against, only the model name typed in Settings, so its output is normalized by `model_json` instead), `model_json.rs` (the single entry point for parsing any LLM response — strict parse first, then a string-aware `repair` pass for the sloppiness models emit: trailing commas before `}`/`]`, `//` and `/* */` comments, raw control chars inside strings. Truncated responses are deliberately NOT salvaged — half a plan silently rendered is worse than a failed run), `edit_service.rs` (background edit pipeline orchestration, in-memory `EditJobMap`; a plan that still won't parse is re-requested once with the parse error attached rather than discarding the transcription pass), `overlay_service.rs` (the bundled "Subscribe" overlay — a transparent GIF embedded via `include_bytes!` from `assets/overlays/` and written out to `<app-data>/overlays/` on startup; exposes the built-in overlay list), `embedding_service.rs` (semantic search — builds a text "document" per video/production, embeds + caches vectors in `video_embeddings`/`production_embeddings`, ranks by cosine similarity; background reindex via in-memory `SearchIndexMap`, incremental via a per-document `content_hash` + `model` guard; pure helpers `assemble_video_doc`/`assemble_production_doc`/`cosine_similarity`/`embedding_to_blob` are unit-tested).
...
1. User adds raw takes to a production, then POSTs a script (+ optional instructions) to `/api/productions/{id}/edit`. Productions carry a `production_type` (`"long"` | `"short"`, migration `009`, alongside a `published_at` date stamped when marked published). **Short-form productions may omit the script**: the pipeline then plans a *cleanup cut* from the transcript alone via `AiSettings.short_edit_prompt` (cut false starts, repeated content, filler, dead air) instead of `edit_prompt`. The VideoModal's "✂️ Create Short" button (portrait videos, desktop) wraps a take in a short-form production and opens this same pipeline; portrait frames get a bolder, higher-margin burned-caption style preset (`extract_clip_segment`)
...
- **Video ↔ VideoEmbedding** / **Production ↔ ProductionEmbedding**: 1:1 (`video_embeddings`/`production_embeddings`, cascade delete, migration `010`) — the cached semantic-search vector (little-endian f32 BLOB) plus its `content_hash`, `model`, and `dim`

### Key Data Flow: Semantic search
1. `POST /api/search/reindex` (body `{transcribe_missing?, describe_visuals?}`) spawns a background thread (own current-thread Tokio runtime, like the edit pipeline; progress in the in-memory `SearchIndexMap`, polled via `/api/search/reindex/status/{id}`). Optional pre-passes: `transcribe_missing` transcribes every video with no transcript yet (extract audio → configured transcription provider → `ai_service::upsert_transcript`; silent clips skipped); `describe_visuals` captions every video with no visual description by sending a few of its **thumbnails** to the multimodal text/LLM provider (`ai_service::describe_video_frames` → `upsert_visual_description`, stored in `ai_generations.visual_description`, migration `011`). It then builds a text "document" for every video that has describable text — filename (only when descriptive, not a bare camera code like `GX011916.MP4`) + metadata + tags + productions + transcript (from `ai_generations`/edit-pipeline `transcripts_json`) + visual description; content-less clips are skipped and their stale embeddings pruned — and for each production (title + platform + latest script/copy + take transcripts), then embeds only those whose `content_hash`+`model` changed (incremental) and upserts the vectors.
2. `GET /api/search/semantic?q=…&type=videos|productions` embeds the query with the configured provider (`AiSettings.embedding_provider`/`embedding_model`, default OpenAI `text-embedding-3-small`), then ranks the cached vectors for the current model by cosine similarity (`embedding_service::rank_videos`/`rank_productions`) and returns the top-N (with an `index_empty` flag when nothing is indexed yet, and a `weak_match` flag — via the testable `is_weak_ranking` — when the top results are all tied, i.e. the query matched nothing specific; the UI shows a "no strong matches" banner).
3. Text-only (no vision): a purely visual query only matches when some text — notes/tags/transcript — describes it. Switching the embedding provider/model requires a rebuild (vectors from different models aren't comparable). Frontend: a **✨ Semantic** toggle on the videos page (`page.tsx`, submit-on-Enter) and in `ProductionManager`; the reindex UI + coverage live in **Settings → AI / LLM**.
