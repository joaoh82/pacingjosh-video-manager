...
- **Services** (`services/`): Business logic — `scanner.rs` (background directory scanning), `ffmpeg_service.rs` (metadata/thumbnails/clip extraction + concat + overlay compositing, with injectable `FfmpegPaths` via `set_ffmpeg_paths()`), `video_service.rs`, `search_service.rs` (keyword `LIKE` filtering + sort/paginate), `production_service.rs`, `ai_service.rs` (transcription incl. ElevenLabs/OpenAI/Gemini + LLM completion + `embed_texts` batch embeddings via OpenAI/Gemini + `describe_video_frames` vision captioning via the multimodal text provider; the OpenAI/Gemini completion paths request JSON mode, while Anthropic has no `response_format` — so it prefills the assistant turn with `{` to force a bare JSON object), `model_json.rs` (the single entry point for parsing any LLM response — strict parse first, then a string-aware `repair` pass for the sloppiness models emit: trailing commas before `}`/`]`, `//` and `/* */` comments, raw control chars inside strings. Truncated responses are deliberately NOT salvaged — half a plan silently rendered is worse than a failed run), `edit_service.rs` (background edit pipeline orchestration, in-memory `EditJobMap`; a plan that still won't parse is re-requested once with the parse error attached rather than discarding the transcription pass), `overlay_service.rs` (the bundled "Subscribe" overlay — a transparent GIF embedded via `include_bytes!` from `assets/overlays/` and written out to `<app-data>/overlays/` on startup; exposes the built-in overlay list), `embedding_service.rs` (semantic search — builds a text "document" per video/production, embeds + caches vectors in `video_embeddings`/`production_embeddings`, ranks by cosine similarity; background reindex via in-memory `SearchIndexMap`, incremental via a per-document `content_hash` + `model` guard; pure helpers `assemble_video_doc`/`assemble_production_doc`/`cosine_similarity`/`embedding_to_blob` are unit-tested).
