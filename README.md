...
- **Subscribe & Overlays** - Drop a transparent **GIF or image** (a built-in **Subscribe** button bug, or any file of your own — a "Like" reminder, a logo sting, etc.) on top of the final video. Each overlay is **auto-placed in the longest pause where you're not talking** (driven by the same word timestamps that power music ducking) and uses its **native transparency**, so it floats cleanly over your footage — no white box, no chroma-key artifacts. GIFs loop and still images hold for their on-screen window. Per-overlay on-screen position and size; placement re-flows automatically on every re-render. A **live placement preview** shows each overlay right on the timeline's player as you scrub or play — so you can judge position, size, and timing **before** committing to a re-render
...
4. Set:
   ...
   - **Overlays** — click **🔔 Add Subscribe** for the built-in subscribe-button GIF, or **➕ Add image/GIF…** to pick your own transparent GIF/PNG. Each one is dropped into the longest pause where you're not talking, using its native transparency. Tune position and size per overlay.
5. Click **Run pipeline**. The app will, in order:
   ...
   - Cut each clip (burning in captions when enabled),
   - Stitch the final clip with FFmpeg (mixing in music if provided), and
   - Composite any overlay GIFs/images into the pauses (native transparency).
