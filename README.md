...
- **Subscribe & Overlays** - Drop a short snippet (a built-in **Subscribe** button bug, or any video file of your own — a "Like" reminder, a logo sting, etc.) on top of the final video. Each overlay is **auto-placed in the longest pause where you're not talking** (driven by the same word timestamps that power music ducking), and its solid background is **chroma-keyed out** so it floats over your footage with no white box. Per-overlay controls for background removal, on-screen position, and size; placement re-flows automatically on every re-render
...
4. Set:
   ...
   - **Overlays** — click **🔔 Add Subscribe** for the built-in subscribe-button snippet, or **➕ Add overlay…** to pick your own video. Each one is dropped into the longest pause where you're not talking, with its background removed. Tune the background removal, position, and size per overlay.
5. Click **Run pipeline**. The app will, in order:
   ...
   - Composite any overlay snippets into the pauses (background keyed out).
